...
ALTER TABLE trading_positions 
ADD COLUMN IF NOT EXISTS close_initiated_at TIMESTAMP WITH TIME ZONE;

-- Add index for better performance when querying closing positions
CREATE INDEX IF NOT EXISTS idx_trading_positions_closing_status 
ON trading_positions(status, close_initiated_at) 
