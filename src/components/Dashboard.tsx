...
import { shareTradeResults } from "../utils/shareUtils";
import {
  userProfileService,
  WithdrawalRequest,
  supabase,
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      if (txid) {
        ...
        console.log(
          `Platform SOL balance: ${currentSOLBalance.toFixed(
            4
          )} + ${amount.toFixed(4)} = ${newPlatformSOLBalance.toFixed(4)} SOL`
        );
        ...
        console.log(
          `User now has ${newPlatformSOLBalance.toFixed(
            4
          )} SOL deposited on platform`
        );
      }
    } catch (error: any) {
        ...
        try {
          ...
          if (closedPosition && !error) {
            ...
            if (
              ...
              console.log("🧮 Calculated P&L from prices:", {
                entryPrice,
                exitPrice,
                amount,
                realPnL,
            ...
            setTimeout(() => {
              console.log(
                "🚀 Showing ShareGainsPopup with REAL trade data:",
                realTradeData
              );
            ...
            console.log(
              "⚠️ Could not find closed position, using fallback data"
            );
            ...
            if (position) {
              ...
              const fallbackPnL =
                typeof position.current_pnl === "number"
                  ? position.current_pnl
                  : parseFloat(position.current_pnl || "0");
              const collateralSol =
                typeof position.collateral_sol === "number"
                  ? position.collateral_sol
                  : parseFloat(position.collateral_sol || "0");
              const fallbackData = {
                tokenSymbol: position.token_symbol,
                direction: position.direction,
                leverage: position.leverage,
                ...
                pnlPercentage:
                  fallbackPnL !== 0 ? (fallbackPnL / collateralSol) * 100 : 0,
